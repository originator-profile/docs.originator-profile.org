...
| `type` | `string` | **REQUIRED.** CA のタイプ名です。既存の型は [Content Attestation Type Registry](./ca-model/index.mdx#registry) に登録されており、必要に応じて拡張して新しい型を定義できます。                         |
...
「OP Inspector」では、CA の検証時、次のエラーを返却することがあります。
