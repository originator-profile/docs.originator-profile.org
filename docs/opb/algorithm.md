...
- ES384: 192 ビット
- ES512: 256 ビット
- PS256: ~128 ビット [^rsa-key-length]
- PS384: ~192 ビット [^rsa-key-length]
- PS512: ~256 ビット [^rsa-key-length]

[^rsa-key-length]: 鍵長による。鍵長 3072 ビットの場合: 128 ビット
...
Originator Profile では、運用ミスなどのセキュリティインシデントにより、署名鍵の漏洩などの前述の署名鍵の保護および保証要件に適合しない状況に至った場合には、鍵の交換と、それに伴う発行済み全 VC の再発行および再設置が必要になります。このような事態を避けるためには、原理的に署名鍵の取り出しが不可能な仕組みを利用すべきです。具体的な手段としては、たとえば Key Management Service (KMS) や Hardware Security Module (HSM) などが挙げられます。
