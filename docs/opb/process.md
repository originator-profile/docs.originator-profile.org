...
OPB の草案を提出した時点～遅くとも OPB の草案に対するレビューが完了し、マージの承認が得られた時点で、以下のいずれかの対応を行う必要があります。

1. 草案に対応する実装を[リファレンス実装リポジトリ](https://github.com/originator-profile/originator-profile)の Pull Request として提出する
2. 実装のボリューム等の要因により、1. が難しい場合は以下を草案上に記載する
   - リファレンス実装リポジトリに起票した実装の Issue へのリンク
   - 実装状況の確認方法（例：「実装状況については \[Issue #XXX](リンク) を参照してください」）
   - 実装が完了した時点（実装 Pull Request がマージされた時点）で記載内容を削除する

これにより、仕様と実装の対応関係を明確にし、OP 利用者が現在の実装と対応している仕様を容易に判別できるようにします。

### アクティベーション

GitHub Pull Request がデフォルトブランチにマージされると、その OPB はアクティブであるとみなします。
OPB がアクティブであるということは、コミュニティが OPB に基づいた実装を進めることに合意しており、実装が進行中または完了していることを意味します。
