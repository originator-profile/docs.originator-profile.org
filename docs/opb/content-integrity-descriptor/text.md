...
| ------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `type`        | `string` | **REQUIRED.** Content Integrity Descriptor のタイプです。必ず `TextTargetIntegrity` でなければなりません (MUST)。                                                                                                                                                                      |
...
2. 対象要素の [descendant text content](https://dom.spec.whatwg.org/#concept-descendant-text-content) を取得します。それは要素の `textContent` 属性の値です。もし仮に `null` が得られた場合は、その対象を空の文字列 ("") に変換します。
