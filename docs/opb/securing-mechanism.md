...
運用ミスやセキュリティインシデントにより署名鍵の漏洩が判明した場合、鍵の交換と、それに伴う発行済み全 VC の再発行および再設置が必要になります。このような事態を避けるためには、原理的に署名鍵の取り出しが不可能な仕組みを利用すべきです。具体的な手段としては、たとえば Key Management Service (KMS) や Hardware Security Module (HSM) などが挙げられます。
