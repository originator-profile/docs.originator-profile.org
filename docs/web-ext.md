...
拡張機能のアイコンをクリックし、ケバブメニュー（︙ 縦三点アイコン）から「詳細情報」を選択すると、検証結果の詳細画面が表示されます。この画面には以下の情報が含まれます。

#### 検証結果

検証に成功した場合、発信者の情報（名称や画像など）が表示されます。

検証に失敗した場合、エラー情報がツリー形式で表示されます。

#### エラーの詳細

![エラーの詳細ツリーの例](./assets/error-details-tree.webp)

検証エラーが発生した場合、「エラーの詳細」ツリーを展開すると、エラーコードとメッセージを確認できます。

- 「▶」をクリックしてツリーを展開します。
- **Code**: エラーコードが表示されます。エラーコードの意味は[エラーリファレンス](error-reference/)を参照してください。
- **payload**: エラーの詳細データが JSON 形式で表示されます。
