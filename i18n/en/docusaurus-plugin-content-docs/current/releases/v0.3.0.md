...
- **BREAKING CHANGE**: Replaced the `url` property of `WebsiteProfile` with `allowedOrigin`.
  - From October 1, 2026, onwards, it will no longer be possible to verify VCs that use the `url` property of WebsiteProfile. Please consider migrating.

## 🧪 Testing & Quality

- **E2E test environment improvements** - Properly handle settings such as `REGSTRY_OPS` during E2E testing in the CI environment
...
- **Migration** - Migrated from tsup to tsdown
